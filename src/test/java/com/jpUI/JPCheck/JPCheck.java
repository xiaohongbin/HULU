package com.jpUI.JPCheck;

import com.aventstack.extentreports.ExtentTest;
import com.hulu.check.publicCheck;
import com.jpUI.method.ElementMethod;
...
//        this.waitElement(driver,helloTips);
...
//    1线男 在线状态
    public void onlineStatus(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste1(driver, qudaohao);
        this.waitElement(driver, iv_user.get(0));
        this.clickIVUser();
        if (this.doesWebElementExist(tips)) {
...
//      再次点击页面空白处，防止有头像多张时，消除 滑动查看照片提示
    ...
    //    3线男 查看视频
    public void video3(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste3(driver, qudaohao);
        this.waitElement(driver, iv_user.get(0));
        this.clickIVUser();
        if (this.doesWebElementExist(tips)) {
...
//      再次点击页面空白处，防止有头像多张时，消除 滑动查看照片提示
    ...
    //    3线男 查看更多资料
    public void more3(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste3(driver, qudaohao);
        this.waitElement(driver, iv_user.get(0));
        this.clickIVUser();
        if (this.doesWebElementExist(tips)) {
...
//      再次点击页面空白处，防止有头像多张时，消除 滑动查看照片提示
    ...
    //    1线男 查看更多资料
    public void more1(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste1(driver, qudaohao);
        this.waitElement(driver, iv_user.get(0));
        this.clickIVUser();
        if (this.doesWebElementExist(tips)) {
...
//    搜索初始条件
    ...
    public void initSeachCondition() {
        ...
        String age;
        String height;
        if (!this.getTopView().equals("精选推荐")) {
            Assert.fail("未进入“发现”模块");
        } else {
            log.info("成功进入“发现”模块");
        }
        this.clickSearchBtn();
...
//        System.out.println(list_age.size());
        ...
        //UI差异化，list_area,代表list_height 身高
        if (list_area.size() > 0) {
            for (int i = 0; i < list_area.size(); i++) {
                if (!list_area.get(i).getText().equals(height)) {
//                System.out.println(list_age.get(i).getText());
    ...
    public void seachAll(AndroidDriver driver, String qudaohao) throws Exception {
        ...
        this.clickHeightDown();
        this.clickBtnok();
        String local_view = this.getAreaText();
        this.clickSaveBtn();
        this.waitElement(driver, topview);
        this.swipeToDown(driver);
        this.waitElementInvisibility(header_content);
...
//      再次点击页面空白处，防止有头像多张时，消除 滑动查看照片提示
        ...
        if (!this.doesWebElementExist(daCall)) {
...
//        信箱模块上方 不为topview，
//        this.waitElement(driver, title_name);//新UI掉标题
    ...
    //    信息未读数量
    public void mailboxNum(AndroidDriver driver, String qudaohao) throws Exception {
...
//        }
    ...
    //    信箱上方完善信息
    public void mailboxPersonData(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste1(driver, qudaohao);
        this.clickMailBoxIcon();
        this.waitElement(driver, personal_data);
        this.clickPersonalData();
        this.waitElement(driver, title_name);
        log.info("判断是否进入 完善信息页面");
...
//        this.clickNearIcon();
//        this.waitElement(driver, title_name);//附近模块没有title
        if(this.doesWebElementExist(helloTips)){
            this.clickHelloTips();
        }
        if (!this.doesWebElementExist(nearTouxiang)) {
            Assert.fail("附近 未弹出头像框");
        }
        if (!this.doesWebElementExist(nearNickname)) {
            Assert.fail("头像框中无姓名");
        }
        if (!this.doesWebElementExist(nearAge)) {
            Assert.fail("头像框中无年龄");
        }
        if (!this.doesWebElementExist(nearDistance)) {
            Assert.fail("头像框中无距离");
        }
        this.clickNearUnlike();
        if(doesWebElementExist(nearLike)){//避免被遮挡
            this.clickNearLike();
        }
        String name = this.getNearNickname();
        this.clickNearTouxiang();
        if (this.doesWebElementExist(tips)) {
            this.clickMonitor1(driver);
        }
//        等待头像左下角的 数量是否出现1/**，用于判断整个页面是否加载完成
...
//        this.clickAskBtn();
        if ((!this.doesWebElementExist(ask_button))&&(!this.doesWebElementExist(daCall))) {
//        if (!this.doesWebElementExist(ask_button)) {
...
//        System.out.println(this.getMemberInfo());
    ...
    public void myCondition(AndroidDriver driver, String qudaohao) throws Exception {
        ...
        Thread.sleep(1000);
        this.waitElementInvisibility(header_content);
        System.out.println("iv_user.size():"+iv_user.size());
        for (int i = 0; i < iv_user.size(); i++) {
            System.out.println("iv_user:第"+i+"个-----"+iv_user.get(i));
            if(i>3){
                this.swipeToUp(driver);
            }
            iv_user.get(i).click();
            ...
            this.clickMonitor1(driver);
            Thread.sleep(1000);
            System.out.println("this.getMemberInfoText():"+this.getMemberInfoText());
            if (!this.getMemberInfoText().contains(local_view)) {
                Assert.fail("缘分页中,第" + (i + 1) + "个用户，地区与征友条件中不一致");
            }
            this.keyBack(driver);
    ...
    //    1线 我的个人问答
    public void myQuestion(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste1(driver, qudaohao);
        this.clickMeIcon();
...
//    收信宝
    public void reciveLetter1(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste1(driver, qudaohao);
        this.clickMeIcon();
        this.clickMemberCenter();
...
//    咪咕
    public void migu(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste1(driver, qudaohao);
        this.clickMeIcon();
        this.clickMemberCenter();
        this.clickMigu();
        this.waitElement(driver, title_name);
        log.info("判断是否进入咪咕动漫");
        if (!this.getTitleName().contains("动漫")&&!this.getTitleName().contains("会员")) {
            Assert.fail("未进入咪咕");
        }
    }

    //    1线男，会员中心 可购买项
    public void VipCenter1() {
        log.info("判断是否进会员中心");
...
//        this.waitElement(driver, title_name);
    ...
    public void VipPrice3(AndroidDriver driver) throws Exception {
        ...
        if (!this.doesWebElementExist(newvip)) {
            Assert.fail("3线男 皇冠特权 领取vip新手礼包不存在");
        }
...
*/
    ...
    public void receiveLetterPrice1(AndroidDriver driver) throws Exception {
        ...
        Thread.sleep(2000);
        System.out.println("activity:"+this.getCurrentActivity(driver));
        if (this.getCurrentActivity(driver).contains("AlipayUserLoginActivity")) {
            log.info("1天展示启动支付宝");
        }else if (this.getCurrentActivity(driver).toLowerCase().contains("alipay")) {
            log.info("1天展示启动支付宝");
        }else if(this.getCurrentActivity(driver).toLowerCase().contains("aliuserguideactivity")){
            log.info("1天展示启动支付宝");
        }else
        ...
        if(!this.doesWebElementExist(yinlianshortcutpay)){
            Assert.fail("1天展示没有银联快捷支付");
        }else log.info("1天展示显示银联快捷支付");
        ...
        if(!this.doesWebElementExist(yinlianshortcutpay)){
            Assert.fail("3天展示没有银联快捷支付");
        }else log.info("3天展示显示银联快捷支付");
        this.clickBtnLeft();
        this.waitElement(driver, title_name);
        this.clickReload();
        this.clickDay7();
        ...
        if(!this.doesWebElementExist(yinlianshortcutpay)){
            Assert.fail("7天展示没有银联快捷支付");
        }else log.info("7天展示显示银联快捷支付");
        this.clickBtnLeft();
        this.waitElement(driver, title_name);
        this.clickReload();
        this.clickDay15();
        ...
        if(!this.doesWebElementExist(yinlianshortcutpay)){
            Assert.fail("15天展示没有银联快捷支付");
        }else log.info("15天展示显示银联快捷支付");
        log.info("收信宝测试通过");
    }
   ...
   /* //    本地图片上传头像
    public void upLoadPic(AndroidDriver driver) {
        this.clickReload();
        this.clickUploadPic();
        this.clickLocalPic();
        this.clickImg();
