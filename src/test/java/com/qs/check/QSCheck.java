...
public class QSCheck extends ElementMethod {
    ...
    public void protocal() throws Exception {
        ...
        this.clickBtnLeft();
        if (!this.doesWebElementExist(login)) {
...
//            log.info("判断打招呼后，进入空间，是否已打招呼");
//            if (!this.getAskButtonText().equals("已打招呼")) {
//                Assert.fail("在列表上点击“打招呼”后，进入空间页中，未标注为 已打招呼");
//            }
...
//        this.swipeToDown(driver);
        ...
        if(this.doesWebElementExist(more_button)){
            this.clickMoreBtn();
            Thread.sleep(1000);
            log.info("判断是否提示 完善资料");
            if (!this.doesWebElementExist(age_dialog_title)) {
                Assert.fail("1线男，点击“更多详细资料”，未提示完善资料");
            }else{
                this.clickSure();
                this.Data(driver);
                if(!this.doesWebElementExist(more_text)){
                    Assert.fail("1线男，完善资料后，无法查看 更多详细资料");
                }else{
...
//    完善个人资料方法
    public  void Data(AndroidDriver driver) throws Exception{
        this.clickHeight();
        this.clickBtnok();
        this.clickWeight();
        this.clickBtnok();
        ...
        this.clickInfoSave();
        Thread.sleep(1000);
        //      5.6.0 三线默认已选择 个性爱好,强制切回以兼容一线
        this.clickHobbyTitle();
        this.clickProperty();
        this.swipeToUp(driver);
        Thread.sleep(1000);
        this.clickHobby();
...
//        牵手 5.6.0需要滑动部分
    ...
    //    1线 我的访客
    public void myVisitor1(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste1(driver, qudaohao);
        this.clickMeIcon();
        this.waitElement(driver, memberinfo);
        this.swipeToUp(driver);
    ...
    //    诚信等级
    public void myIntegrity1(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste1(driver, qudaohao);
        this.clickMeIcon();
        this.waitElement(driver, memberinfo);
        this.swipeToUp(driver);
...
//        this.clickBtnLeft();
    ...
    public void seachArea(AndroidDriver driver, String qudaohao) throws Exception {
        ...
        log.info("牵手5.6搜索结果列表中不在显示地区");
//        if (list_area.size() > 0) {
//            for (int i = 0; i < list_area.size(); i++) {
////                牵手 5.6 地区显示为 类似 上海市区 数据，由equal改为 contains
//                if (!list_area.get(i).getText().contains(area)) {
...
//                System.out.println(list_age.get(i).getText());
                if (!age.contains(list_age.get(i).getText())) {
    ...
    public void seachHeight(AndroidDriver driver, String qudaohao) throws Exception {
        ...
        this.clickHeightDown();
        this.clickBtnok();
        height = this.getHeightText().replace("cm","");
        System.out.println(height);
        log.info("身高设置条件"+height);
        this.clickSaveBtn();
        this.waitElement(driver, topview);
        this.swipeToDown(driver);
        this.waitElementInvisibility(header_content);
        ...
        if (list_height.size() > 0) {
            for (int i = 0; i < list_height.size(); i++) {
                System.out.println(list_height.get(i).getText());
                if (list_height.get(i).getText().contains(height)) {
                    log.info("结果列表中，存在符合条件的身高信息");
                }else if(!list_height.get(i).getText().contains(height)) {
                    log.info("结果列表中，出现不符合条件的身高信息");
                }
            }
        } else if (!this.doesWebElementExist(search_fragment_net_error_tv)) {
            Assert.fail("搜索无结果时，未提示未找到符合要求的人");
...
//            log.pass("该女用户只有一张照片");
//        }
        if (this.doesWebElementExist(image2)) {
            this.clickZonePic2();
            log.info("判断是否弹出升级vip提示");
            if (!this.doesWebElementExist(dialog_bg)) {
                Assert.fail("点击被锁照片，未弹出升级vip提示");
            }
            this.moreGetVip3(driver);
        }else{
            log.pass("该女用户只有一张照片");
...
//      再次点击页面空白处，防止有头像多张时，消除 滑动查看照片提示
    ...
    //    1 女用户 空间页 查看更多资料
    public void womanMore1(AndroidDriver driver, String qudaohao, String account, String password) throws Exception {
...
//        Thread.sleep(1000);
        ...
        if(this.doesWebElementExist(more_button)){
            ...
            log.info("判断是否提示 完善资料");
            if (!this.doesWebElementExist(age_dialog_title)) {
                Assert.fail("1线女，点击“更多详细资料”，未提示完善资料");
            }else{
                this.clickSure();
                this.Data(driver);
                if(!this.doesWebElementExist(more_text)){
                    Assert.fail("1线女，完善资料后，无法查看 更多详细资料");
                }else{
                    log.info("男用户资料不完整，不显示更多详细资料按钮");
                }
            }
        }else{
            log.info("男用户资料不完整，不显示更多详细资料按钮");
...
//        Thread.sleep(1000);
        ...
        if(this.doesWebElementExist(more_button)){
            ...
            log.info("判断是否提示 完善资料");
            if (!this.doesWebElementExist(age_dialog_title)) {
                Assert.fail("3线女，点击“更多详细资料”，未提示完善资料");
            }else{
                this.clickSure();
                this.Data(driver);
                if(!this.doesWebElementExist(more_text)){
                    Assert.fail("3线女，完善资料后，无法查看 更多详细资料");
                }else{
                    log.info("男用户资料不完整，不显示更多详细资料按钮");
                }
            }
    ...
    //    女用户 空间页 下方广告
    public void womanAdvertisement1(AndroidDriver driver, String qudaohao, String account, String password) throws Exception {
        this.womanRegiste1(driver, qudaohao);
        this.waitElement(driver, iv_user.get(0));
        this.clickIVUser();
//        if (this.doesWebElementExist(tips)) {
...
//        }
    ...
    //女用户 判断无付款入口
    public void womanMemberCenter1(AndroidDriver driver, String qudaohao, String account, String password) throws Exception {
        this.womanRegiste1(driver, qudaohao);
        this.clickMeIcon();
...
//        this.clickBtnok();
        ...
        this.waitElement(driver, yindaoNear);
        this.clickYindaoTouXiang();
        Thread.sleep(1000);
        if (!this.doesWebElementExist(head_map)) {
            Assert.fail("附近 点击头像，未弹出头像框");
        }
