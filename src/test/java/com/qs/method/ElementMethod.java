...
*/
    ...
    public void clickLoginRegistButton(){
        log.info("点击登录页面 左上角 返回");
...
//        TouchAction(driver).longPress(700, 650).waitAction(duration).moveTo(50, 650).perform();
    ...
    public void clickNearHello(){
        log.info("点击附近人 打招呼按钮");
        nearhello.get(0).click();
    }
...
//   滑动部分（个人问答，我的标签 在页面中间）
    public void swipeToUp1(AndroidDriver driver) {
        log.info("向上滑动屏幕");
        Duration duration = Duration.ofSeconds(1);
        int width = driver.manage().window().getSize().width;
        int height = driver.manage().window().getSize().height;
        TouchAction action1 = new TouchAction(driver).press(width / 2, 1050).waitAction(duration).moveTo(width / 2, 500).release();
        action1.perform();
    }
    /* 公共
...
*/
    ...
    public void clickYindaoTouXiang(){
        log.info("三线男注册引导LBS地图页面，点击女用户头像");
        yindaoTouxiang.get(0).click();
    }
