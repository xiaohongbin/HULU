...
public class PLPCheck extends ElementMethod {
...
//    }
    ...
    //    搜索
    //    搜索条件初始验证
    public void initSeachCondition(AndroidDriver driver, String qudaohao) throws Exception{
        this.manRegiste1(driver, qudaohao);
...
//        信箱模块上方 不为topview，
    ...
    public void mailboxPersonData3(AndroidDriver driver, String qudaohao) throws Exception {
        ...
        this.waitElement(driver, title_name);
        log.info("判断是否返回 信箱");
        if (!this.getTitleName().contains("信箱")) {
            Assert.fail("点击 左上角返回，未回到“信箱”模块");
        }
    }
...
//        this.manRegiste1(driver, qudaohao);
        this.manLogin(driver,"902457266","aaa123456",1);
        this.clickMeIcon();
...
//        this.manRegiste3(driver, qudaohao);
        this.manLogin(driver,"902458455","aaa123456",3);
        this.clickMeIcon();
        this.clickMemberCenter();
        this.waitElement(driver, title_name);
//        this.VipCenter();
...
//        this.manRegiste1(driver, qudaohao);
        this.manLogin(driver,"902458365","aaa123456",1);
        this.clickMeIcon();
        this.clickMemberCenter();
        this.clickWriteLetter();
...
//      再次点击页面空白处，防止有头像多张时，消除 滑动查看照片提示
    ...
    //    上传照片
    public void uploadPic(AndroidDriver driver, String qudaohao) throws Exception {
        this.manRegiste3(driver, qudaohao);
        this.clickMeIcon();
        log.info("判断是否提示未上传照片");
    ...
    public void migu(AndroidDriver driver, String qudaohao) throws Exception {
        ...
        this.waitElement(driver, memberinfo);
        this.swipeToUp(driver);
        this.waitElement(driver, miguvip);
        this.clickMiGuVip();
        this.waitElement(driver, title_name);
        this.clickReload();
        this.clickDingGouMiGu();
        if (!this.doesWebElementExist(migupay)) {
            Assert.fail("订购咪咕vip，未弹出付费窗口");
        }
    }
    //      豆币，免密开通及两次支付引导
...
//        }
        ...
        this.clickBtnLeft();
        this.clickYuanFenIcon();
        if(this.doesWebElementExist(btn_super_say_hello)){
            this.clickduiduipeng(driver);
            Thread.sleep(2000);
            log.info("判断开通免密后，对对碰中是否提示购买会员");
            if (!this.getOKText().equals("免密解锁：99元90天畅聊")) {
...
//        }
        ...
        wui.appendFile("E:\\PLP.csv",this.getRegisterDate()+","+qudaohao+","+address+","+ID1+"\r\n");
